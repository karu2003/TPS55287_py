...
class TSP55287:
    ...
    REGISTERS = {
        ...
        "REF_MSB": {
            "address": 0x01,
            "description": "Reference Voltage MSB (10-8 bits)",
        },
    ...
    CDC_VALUES = {
        ...
        "CDC_OPTION": {
            0b0: "Internal CDC compensation",
            0b1: "External CDC compensation",
        },
        "CDC": {
            0b000: "0 V",
            0b001: "0.1 V",
    ...
    MODE_VALUES = {
        ...
        "FSWDBL": {
            0b0: "Keep switching frequency unchanged",
            0b1: "Double switching frequency in buck-boost mode",
        },
        "HICCUP": {
            0b0: "Disable hiccup during short circuit protection",
            0b1: "Enable hiccup during short circuit protection",
        },
        "DISCHG": {
            0b0: "Disabled VOUT discharge in shutdown mode",
            0b1: "Enable VOUT discharge in shutdown mode",
        },
        "Force_DISCHG": {
            0b0: "Disabled VOUT discharge FET",
            0b1: "Force enable VOUT discharge FET",
        },
        "FPWM": {
            0b0: "PFM operating mode at light load",
            0b1: "FPWM operating mode at light load",
        },
    }
    ...
    def print_registers(self):
        ...
        for name, info in self.REGISTERS.items():
            try:
                ...
                print(
                    f"Register {name} (Address {info['address']:#02x}): {value:#02x} - {info['description']}"
                )
            except Exception as e:
    ...
    def set_ref_voltage_in_volts(self, voltage):
        ...
        if self.verbose:
            print(
                f"REF set to {voltage:.3f} V (REF_LSB={ref_lsb:#02x}, REF_MSB={ref_msb:#02x})"
            )
    ...
    def set_iout_limit(self, enable, setting):
        ...
        if self.verbose:
            print(
                f"IOUT_LIMIT set: Current_Limit_EN={enable}, Current_Limit_Setting={setting} (VISP-VISN={setting * 0.5:.1f} mV)"
            )
    ...
    def calculate_current(self):
        ...
        current = (
            voltage_mv / 1000
        ) / self.rshunt  # Convert mV to V and calculate current
        return current
    ...
    def set_vout_sr(self, ocp_delay, sr):
        ...
        if self.verbose:
            print(
                f"VOUT_SR set: OCP_DELAY={ocp_delay} ({self.OCP_DELAY_VALUES.get(ocp_delay, 'Unknown')}), "
    ...
    def set_vout_fs(self, fb, intfb):
        ...
        if self.verbose:
            print(
                f"VOUT_FS set: FB={fb} ({'External feedback' if fb else 'Internal feedback'}), "
    ...
    def get_cdc(self):
        ...
        return {
            ...
            "OCP_MASK": ocp_mask,
            "OCP_MASK_DESCRIPTION": self.CDC_VALUES["OCP_MASK"].get(
                ocp_mask, "Unknown"
            ),
            "OVP_MASK": ovp_mask,
            "OVP_MASK_DESCRIPTION": self.CDC_VALUES["OVP_MASK"].get(
                ovp_mask, "Unknown"
            ),
            "CDC_OPTION": cdc_option,
            "CDC_OPTION_DESCRIPTION": self.CDC_VALUES["CDC_OPTION"].get(
                cdc_option, "Unknown"
            ),
            "CDC": cdc,
    ...
    def set_cdc(self, sc_mask, ocp_mask, ovp_mask, cdc_option, cdc):
        ...
        if self.verbose:
            print(
                f"CDC set: SC_MASK={sc_mask} ({self.CDC_VALUES['SC_MASK'].get(sc_mask, 'Unknown')}), "
    ...
    def get_mode(self):
        ...
        return {
            ...
            "Force_DISCHG_DESCRIPTION": self.MODE_VALUES["Force_DISCHG"].get(
                force_dischg, "Unknown"
            ),
            "FPWM": fpwm,
    ...
    def set_mode(self, oe=0, fswdbl=0, hiccup=1, dischg=0, force_dischg=0, fpwm=0):
        ...
        if self.verbose:
            print(
                f"MODE set: OE={oe} ({self.MODE_VALUES['OE'].get(oe, 'Unknown')}), "
    ...
    def set_vout(self, vout):
        ...
        if intfb_key not in self.VOUT_FS_INTFB_VALUES:
            raise ValueError(
                "Invalid INTFB key. Must be one of 0b00, 0b01, 0b10, 0b11."
            )
        ...
        if self.verbose:
            ...
            print(
                f"INTFB set to {bin(intfb_key)} (Ratio: {self.VOUT_FS_INTFB_VALUES[intfb_key]})"
            )
    ...
    def set_iout_limit_from_current(self, enable, current):
        ...
        if not (0 <= voltage_mv <= 63.5):
            raise ValueError(
                "Calculated voltage exceeds the allowable range (0-63.5 mV)."
            )
        ...
        if self.verbose:
            print(
                f"IOUT_LIMIT set for current {current:.3f} A (VISP-VISN = {voltage_mv:.1f} mV)"
            )
